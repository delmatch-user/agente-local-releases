...
export default function ScaleSetup({ onTokenSet }: ScaleSetupProps) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault()
    if (!token.trim()) {
      setError('Insira o token da balança')
      return
    }
  ...
  return (
    <div className="max-w-md mx-auto mt-8">
      <div className="text-center mb-8">
        ...
        <h2 className="text-2xl font-bold text-slate-800">Configurar Balança</h2>
        <p className="text-slate-600 mt-2">
          Cole o token gerado na seção de Balanças do MiaCardapio
        </p>
      </div>

      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label htmlFor="scale-token" className="block text-sm font-medium text-slate-700 mb-1">
            Token da Balança
          </label>
          <textarea
            id="scale-token"
        ...
        >
          {loading ? 'Conectando...' : 'Conectar Balança'}
        </button>
      </form>
      ...
      <div className="mt-8 p-4 bg-slate-100 rounded-lg">
        ...
        <ol className="text-sm text-slate-600 space-y-1 list-decimal list-inside">
          ...
          <li>Vá em Integrações → Balanças</li>
          <li>Clique em "Gerar Token da Balança"</li>
